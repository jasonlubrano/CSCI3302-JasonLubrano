...
#define DRAWING 0 // if 1 then sparki draws on map
...
unsigned long startTime, endTime, totalLoop, odoTime, loopRemainder;

// used to find the speed, used floats for decimals
float velocity, velDist = 30, velTime, vel = 0.028;
...
float D = 857.25; // changed to m, distance between wheels
float halfLR = 0.0;
...
void followLine() {
  ...
  pose_theta += (rightWheel - leftWheel) * (1 / D) * loopTime * 10; // theta in global FoR
...
void loop() {
  ...
  (totalLoop > 0) ? delay(totalLoop) : delay(loopTime);
}